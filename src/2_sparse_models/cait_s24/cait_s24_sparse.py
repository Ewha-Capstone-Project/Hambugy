# =============================================================================
# cait_s24_sparse.py  ─  CaiT-S24 Sparse TensorRT 엔진 실행 / 속도 측정
#
# 실행:
#   python cait_s24_sparse.py
#
# Nsight Systems 프로파일링:
#   nsys profile --trace=cuda,nvtx -o sparse_profile python cait_s24_sparse.py
# =============================================================================
...
# ── 설정 ─────────────────────────────────────────────────────────────────────
ENGINE_PATH = "cait_s24_model/cait_s24_sparse_fp16.engine"
BATCH_SIZE  = 16
INPUT_HW    = 384
WARMUP      = 50
REPEAT      = 200
...
# ── 실행 ─────────────────────────────────────────────────────────────────────
print(f"Sparse 엔진 로드: {ENGINE_PATH}")
...
with torch.cuda.stream(stream):
    for _ in range(WARMUP):
        if not context.execute_async_v3(stream.cuda_stream):
            raise RuntimeError("Sparse warmup 실행 실패")
...
with torch.cuda.stream(stream):
    for _ in range(REPEAT):
        ...
        if not context.execute_async_v3(stream.cuda_stream):
            raise RuntimeError("Sparse 측정 실행 실패")
        ender.record(stream)
        ender.synchronize()
